...
opts = Optimist.options do
  ...
  opt :dbname, "Database to connect export from", short: "-n", type: :string,
...
@where = []
if OPTS.user
  @user = User.find_by_id( OPTS.user ) || User.find_by_login( OPTS.user )
  if @user
...
if OPTS.observation || OPTS.observation_ids_file
  observation_ids = if OPTS.observation_ids_file
    ...
    CSV.foreach( OPTS.observation_ids_file ) do | row |
      ids << row[0].to_i unless row[0].to_i.zero?
    end
    ids.uniq
  ...
  if ActiveRecord::Base.connection.current_database != OPTS.dbname
    # if we're trying to extract data from a separate database AND we're specifying obs IDs,
    # we need to look for them in that database
...
if OPTS.observed_on
  @observed_on = Date.parse( OPTS.observed_on )
  if @observed_on
...
if OPTS.created_on
  @created_on = Date.parse( OPTS.created_on )
  if @created_on
...
if @where[0].to_s =~ /id IN \(/
  es_cmds << @observations.in_groups_of( 500 ).map do | obs |
    new_where = ["observations.id IN (#{obs.compact.map( &:id ).join( ',' )})"] + @where[1..]
    "RAILS_ENV=production bundle exec rails r \"Observation.elastic_index!( scope: " \
      "Observation.where( '#{new_where.join( ' AND ' )}' ) )\""
  end.join( " && \\\n" )
  es_cmds << @observations.in_groups_of( 500 ).map do | obs |
    new_where = ["observations.id IN (#{obs.compact.map( &:id ).join( ',' )})"] + @where[1..]
    "RAILS_ENV=production bundle exec rails r \"Identification.elastic_index!( scope: " \
      "Identification.joins(:observation).where( '#{new_where.join( ' AND ' )}' ) )\""
  end.join( " && \\\n" )
else
  es_cmds << "RAILS_ENV=production bundle exec rails r \"Observation.elastic_index!( scope: " \
    "Observation.where( '#{@where.join( ' AND ' )}' ) )\""
  es_cmds << "RAILS_ENV=production bundle exec rails r \"Identification.elastic_index!( scope: " \
    "Identification.joins(:observation).where( '#{@where.join( ' AND ' )}' ) )\""
end
...
unless OPTS.skip.include?( "Observation" )
  ...
  cmd = "psql #{OPTS.dbname} -c \"COPY (SELECT #{column_names.join( ', ' )} FROM observations " \
    "WHERE #{@where.join( ' AND ' )}) TO STDOUT WITH CSV\" > #{fname}"
  puts "\t#{cmd}"
  system cmd
  resurrection_cmds << "psql #{OPTS.dbname} -c \"\\COPY observations (#{column_names.join( ', ' )}) " \
    "FROM '#{fname}' WITH CSV\""
end

has_many_reflections = Observation.reflections.select {| _k, v | v.macro == :has_many }
...
unless OPTS.skip.include?( "Photo" )
  ...
  puts "\t#{cmd}"
  system cmd
  resurrection_cmds << "psql #{OPTS.dbname} -c \"\\COPY photo_metadata " \
    "(#{column_names.join( ', ' )}) FROM '#{fname}' WITH CSV\""
end
...
unless OPTS.skip.include?( "ObservationPhoto" )
  ...
  puts "\t#{cmd}"
  system cmd
  resurrection_cmds << "psql #{OPTS.dbname} -c \"\\COPY observation_photos " \
    "(#{column_names.join( ', ' )}) FROM '#{fname}' WITH CSV\""
end
...
unless OPTS.skip.include?( "Sound" )
  ...
  puts "\t#{cmd}"
  system cmd
  resurrection_cmds << "psql #{OPTS.dbname} -c \"\\COPY sounds " \
    "(#{column_names.join( ', ' )}) FROM '#{fname}' WITH CSV\""
end
...
if OPTS.from_user_resurrection && @user
  ...
  puts "  Observation.elastic_index!( scope: Observation.joins( :identifications )." \
