...
( function () {
  document.querySelectorAll( "[data-carousel]" ).forEach( function ( carousel ) {
    ...
    function activeDotIndex() {
      ...
      var centeredIdx = 0; var
        closestDistanceToCenter = Infinity;
    ...
    function scrollToIndex( idx ) {
      var wrappedIdx = ( ( idx % items.length ) + items.length ) % items.length;
      var item = items[wrappedIdx];
      var itemRect = item.getBoundingClientRect();
      var trackRect = track.getBoundingClientRect();
      var offset = track.scrollLeft + itemRect.left - trackRect.left
    ...
    function updateStaticMode() {
      ...
      if ( !fits && wasStatic ) {
        ...
        requestAnimationFrame( function () {
          var initIdx = carousel.dataset.initialIndex != null
            ? parseInt( carousel.dataset.initialIndex, 10 )
            : Math.floor( items.length / 2 );
          var item = items[initIdx];
          var itemRect = item.getBoundingClientRect();
          var trackRect = track.getBoundingClientRect();
          track.scrollLeft = track.scrollLeft + itemRect.left - trackRect.left
            - ( trackRect.width - itemRect.width ) / 2;
          syncDots();
        } );
      }
