...
$( "#import_photos_dialog [data-toggle='tab']" ).on( "show.bs.tab", function ( e ) {
  var $tab = $( e.target );
  var provider = $tab.data( "provider" );
...
function sectionsForTab( current ) {
  ...
  $.getJSON( "/guide_sections/import?provider=" + provider + "&q=" + GUIDE_TAXON.name
      + "&eol_page_id=" + EOL_PAGE_ID + "&guide_taxon_id=" + GUIDE_TAXON.id, function ( json ) {
    current.html( "" );
...
function sectionToHtml( section ) {
  ...
  div.append(
    $( "<a class='right btn btn-default'>" + I18n.t( "import" ) + "</a>" ).click( function ( ) {
      addSection( section );
    } )
  );
...
function rangeToHtml( range ) {
  ...
  div.append(
    $( "<a class='btn btn-default'>" + I18n.t( "import" ) + "</a>" ).click( function ( ) {
      addRange( range );
    } )
  );
