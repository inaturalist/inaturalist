...
export interface IdentificationItemProps {
...
const IdentificationItem = ( {
  ...
  const isDisagreementWithHiddenIdent = ( ) => {
    ...
    if ( !item.previous_observation_taxon_id || !item.disagreement ) return false;
  ...
  const identificationVotes = ( ) => {
    ...
    _.each( item.exemplar_identification.votes, ( v: any ) => {
      if ( v.vote_flag === true ) votesFor.push( v );
      else if ( v.vote_flag === false ) votesAgainst.push( v );
  ...
  let canAgree = false;
  let userAgreedToThis: boolean | undefined;
  if (
    loggedIn
    && item.current
    && item.firstDisplay
  ...
  if ( userCanInteract && item.firstDisplay && !hideCompare ) {
    buttons.push( (
      <button
        ...
        className="btn btn-default btn-sm"
        onClick={e => {
          if ( onClickCompare ) return onClickCompare( e, taxon, observation!, { currentUser } );
          return true;
        }}
      >
  ...
  const className = item.current ? "identification" : "withdrawn";
  ...
  const contents = (
    <div className="identification">
      ...
      <div className="buttons">
        <div className="btn-space">{ buttons }</div>
      </div>
      { !_.isEmpty( idBody ) && (
        <div className="id-body-wrapper">{ idBody }</div>
      ) }
    </div>
  );
  ...
  if ( unresolvedFlags.length > 0 ) {
    ...
    headerItems.push( (
      <span key={`flagged-${item.uuid}`} className="item-status">
        <a
          href={`/identifications/${item.uuid}/flags`}
    ...
    if ( item.category === "maverick" ) {
      ...
      idCategory = (
        <span key={`maverick-${item.uuid}`} className="item-status ident-category" title={idCategoryTooltipText}>
      ...
      idCategory = (
        <span key={`improving-${item.uuid}`} className="item-status ident-category" title={idCategoryTooltipText}>
      ...
      idCategory = (
        <span key={`leading-${item.uuid}`} className="item-status ident-category" title={idCategoryTooltipText}>
          <i className="icon-icn-leading-id" />{ " " }{ I18n.t( "leading" ) }
        </span>
      );
    }
    if ( idCategory ) headerItems.push( idCategory );
  }
  ...
  const elementID = `activity_identification_${item.uuid}`;
  const itemURL = `/identifications/${item.uuid}`;
