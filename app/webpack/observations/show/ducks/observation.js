...
import {
  ...
  fetchNearby,
  setEarlierUserObservations,
  setLaterUserObservations,
  setMoreFromClade,
  setNearby
...
export function renderObservation( observation, options = { } ) {
  return ( dispatch, getState ) => {
    ...
    setTimeout( ( ) => {
      ...
      if (
        ( fetchAll || options.fetchIdentifiers || taxonUpdated )
...
export function review( ) {
  return ( dispatch, getState ) => {
    ...
    if ( !hasObsAndLoggedIn( state ) ) { return; }
    dispatch( setAttributes( {
      reviewed_by: state.observation.reviewed_by.concat( [state.config.currentUser.id] )
    } ) );
...
export function unfollowUser( ) {
  return ( dispatch, getState ) => {
    ...
    const newSubscriptions = _.map( state.subscriptions, s => (
      s.resource_type === "User" ? Object.assign( { }, s, { api_status: "deleting" } ) : s
    ) );
    dispatch( setSubscriptions( newSubscriptions ) );
      ...
      if ( a.uuid === id ) {
        return Object.assign( { }, a, {
          ...
          votes: _.map( a.votes, v => (
            v.user.id === state.config.currentUser.id ? Object.assign( { }, v, { api_status: "deleting" } ) : v
          ) )
        } );
      }
      return a;
    ...
    inatjs.projects.add( payload ).then( ( ) => {
      ...
      dispatch( handleAPIError( e, `Failed to add to project ${project.title}`, {
        onConfirm: ( ) => {
          ...
          dispatch( setAttributes( {
            project_observations: _.filter( currentProjObs, po => ( po.project.id !== project.id ) )
          } ) );
        }
      } ) );
    } );
