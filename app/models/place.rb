...
class Place < ApplicationRecord
  ...
  has_one :place_geometry_without_geom, lambda {
    select( PlaceGeometry.column_names - ["geom"] )
  }, class_name: "PlaceGeometry"

  before_save :calculate_bbox_area, :set_display_name
  after_save :check_default_check_list
  after_save :reindex_projects_later, if: proc {| place | place.ancestry_changed? }
  ...
  validates_uniqueness_of :name, scope: :ancestry, unless: proc {| p | p.ancestry.blank? }
  ...
  requires_privilege :organizer,
    if: proc {| place | place.user && !place.user.is_curator? && !place.user.is_admin? },
    on: :create
  ...
  attr_accessor :html, :updating_bbox
  ...
  PLACE_TYPES = GEO_PLANET_PLACE_TYPES.merge( INAT_PLACE_TYPES ).merge( GADM_PLACE_TYPES ).delete_if do | k, _v |
  ...
  TOWN_LEVEL = 30
  PARK_LEVEL = 100
  ADMIN_LEVELS = [CONTINENT_LEVEL, REGION_LEVEL, COUNTRY_LEVEL, STATE_LEVEL, COUNTY_LEVEL, TOWN_LEVEL,
                  PARK_LEVEL].freeze
  ...
  scope :place_type, lambda {| place_type |
    place_type = PLACE_TYPE_CODES[place_type] if place_type.is_a?( String ) && place_type.to_i.zero?
  ...
  scope :straddles_date_line, lambda {
    where( "swlng > 180 OR swlng < -180 OR nelng > 180 OR nelng < -180 OR (swlng > 0 AND nelng < 0)" )
  }
  ...
  def display_name( options = {} )
    ...
    system_ancestor_names = ancestors.reverse.select do | a |
      [Place::TOWN_LEVEL, Place::STATE_LEVEL, Place::COUNTRY_LEVEL].include?( a.admin_level )
    end
    ancestor_names = system_ancestor_names.map do | a |
      a.code.blank? ? a.name : a.code.split( "-" ).last
    end.compact

    new_name = if admin_level == COUNTY_LEVEL && ancestor_names.include?( "US" )
      "#{name} County"
    else
      name
    end
  ...
  def straddles_date_line?
    return true if swlng.to_f.abs > 180 || nelng.to_f.abs > 180

    swlng.to_f.positive? && nelng.to_f.negative?
  end
  ...
  def validate_with_geom( geom, max_area_km2: nil, max_observation_count: nil )
    if geom.is_a?( GeoRuby::SimpleFeatures::Geometry )
      georuby_geom = geom
      geom = begin
        RGeo::WKRep::WKBParser.new.parse( georuby_geom.as_wkb )
  ...
  def save_geom( geom, max_area_km2: nil, max_observation_count: nil, **other_attrs )
    ...
    begin
      ...
      Rails.logger.error "[ERROR] \tCouldn't save #{place_geometry}: #{e.message[0..200]}"
  ...
  def append_geom( geom, **other_attrs )
    if geom.is_a?( GeoRuby::SimpleFeatures::Geometry )
      geom = begin
        RGeo::WKRep::WKBParser.new.parse( geom.as_wkb )
      rescue StandardError
        nil
      end
    end
    new_geom = geom
    place_geometry.reload
    ...
    save_geom( new_geom, **other_attrs )
  end
  ...
  def points_from_geom( geom )
    { latitude: geom.centroid.y, swlat: geom.envelope.lower_corner.y, nelat: geom.envelope.upper_corner.y }.tap do | h |
      if geom.spans_dateline?
        h[:longitude] = geom.envelope.centroid.x + ( 180 * ( geom.envelope.centroid.x.positive? ? -1 : 1 ) )
        h[:swlng] = geom.points.map( &:x ).select {| x | x.positive? || x < -180 }.min
        h[:nelng] = geom.points.map( &:x ).select {| x | x.negative? || x > 180 }.max
      else
  ...
  # lat/lon coordinates.
  # Options:
  #   <tt>source</tt>: specify a type of handler for certain shapefiles.  Current options are 'census', 'esriworld', and
  #     'cpad'
  #   <tt>test</tt>: (boolean) setting this to +true+ will do everything other than saving places and geometries.
  #   <tt>ancestor_place</tt>: (Place) scope searches for exissting records to descendents of this place. Matching will
  #     be based on name and place_type
  ...
  # Examples:
  #   Census:
  #     Place.import_from_shapefile(
  #       "/Users/kueda/Desktop/tl_2008_06_county/tl_2008_06_county.shp",
  #       place_type: "county",
  #       source: "census"
  #     )
  #
    ...
    RGeo::Shapefile::Reader.open( shapefile_path ) do | file |
      file.each do | shp |
        ...
        if !new_place.source_filename.blank? && !new_place.name.blank?
          existing ||= begin
            Place.where( source_filename: new_place.source_filename ).where( "lower(name) = ?",
              new_place.name.downcase ).first
          rescue StandardError
  ...
  def self.new_from_shape( shape, options = {} )
    ...
    place = Place.new( options.select {| k, _v | Place.instance_methods.include?( "#{k}=".to_sym ) }.merge(
      name: name,
  ...
  def merge( mergee, options = {} )
    ...
    if ( keepers = options[:keep] )
      ...
      keepers.each do | attr_name |
        send( "#{attr_name}=", mergee.send( attr_name ) )
      end
    end
    ...
    elsif mergee.check_list && mergee.check_list.listed_taxa.count.positive?
    ...
    CheckList.where( place_id: id ).each do | cl |
      cl.delay( priority: USER_INTEGRITY_PRIORITY,
        unique_hash: { "CheckList::refresh": cl.id } ).refresh
    end
    check_list&.delay( priority: USER_INTEGRITY_PRIORITY,
      unique_hash: { "CheckList::refresh": check_list.id } )&.refresh
    self
  end
  ...
  def bbox_privately_contains_observation?( obs )
    sw = obs.private_sw_latlon
    ne = obs.private_ne_latlon
    return false unless sw && ne

    bbox_contains_lat_lng?( *sw ) && bbox_contains_lat_lng?( *ne )
  end

  def bbox_publicly_contains_observation?( obs )
    sw = obs.sw_latlon
    ne = obs.ne_latlon
    return false unless sw && ne
  ...
  def self.param_to_array( places )
    ...
    elsif places.is_a?( Array ) && !places.empty?
      case places.first
      when Place
