...
class ObservationField < ApplicationRecord
  ...
  TYPES = %w(text numeric date time datetime taxon dna).freeze
  TYPES.each do | t |
  ...
  def strip_allowed_values
    ...
    self.allowed_values = allowed_values.split( "|" ).map( &:strip ).join( "|" )
    true
  end
  ...
  def reindex_observations_if_name_changed
    ...
    Observation.elastic_index!(
      ids: ObservationFieldValue.where( observation_field_id: id ).pluck( :observation_id ),
      delay: true
    )
  end
