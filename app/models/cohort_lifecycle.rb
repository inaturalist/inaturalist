class CohortLifecycle < ApplicationRecord
  ...
  def self.prepare_cohort_data( raw_cohort_data )
    ...
    raw_cohort_data.each do | cohort |
      ...
      cohort_data[cohort_time][user_id.to_sym] = cohort.attributes.except( "id", "created_at", "updated_at", "cohort", "user_id" ).transform_keys( &:to_sym )
    end
    cohort_data
  end
  ...
  ( ( current_day - window_start) / (60 * 60 * 24 ) ).to_i.times do | i |
    ...
    cohorts.each do | cohort |
      ...
      casual = obs_data.select { |_, v| (v[:needs_id].nil? || v[:needs_id].zero?) && (v[:research].nil? || v[:research].zero?) }.keys
      needs_id = obs_data.select { |_, v| !v[:needs_id].nil? && v[:needs_id].positive? && (v[:research].nil? || v[:research].zero?) }.keys
