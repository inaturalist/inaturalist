...
class Identification < ApplicationRecord
  ...
  blockable_by ->( identification ) { identification.observation.try( :user_id ) }
  ...
  scope :by, ->( user ) { where( "identifications.user_id = ?", user ) }
  scope :not_by, ->( user ) { where( "identifications.user_id != ?", user ) }
  ...
  scope :on, ->( date ) { where( Identification.conditions_for_date( "identifications.created_at", date ) ) }
  ...
  def replace_inactive_taxon
    return true if taxon&.is_active?
  ...
  def update_observation
    ...
    observation.wait_for_index_refresh ||= !wait_for_obs_index_refresh.nil?
  ...
  def update_observation_after_destroy
    ...
    if user_id == observation.user_id
      ...
      ProjectUser.delay( priority: INTEGRITY_PRIORITY,
        run_at: 1.minute.from_now,
        unique_hash: { "ProjectUser::update_taxa_obs_and_observed_taxa_count_after_update_observation": [
          observation.id, user_id
        ] } ).update_taxa_obs_and_observed_taxa_count_after_update_observation( observation.id, user_id )
    end
    observation.skip_identifications = true
    observation.identifications.reload
    Observation.preload_associations( observation, { identifications: [:taxon, :moderator_actions] } )
  ...
  def revisit_curator_identification
    ...
    ).run_revisit_curator_identification( observation_id, user_id )
    true
  end
  ...
  def in_agreement_with?( identification )
    ...
    return true if taxon_id == identification.taxon_id

    taxon.in_taxon? identification.taxon
  end
  ...
  def self.update_categories_for_observation( obs, options = {} )
    unless options[:skip_reload]
      obs = Observation.where( id: obs ).includes( :community_taxon ).first
    end
    return unless obs

    idents = if options[:skip_reload]
      obs.identifications
    else
      Identification.
        includes( :taxon ).
    ...
    idents.sort_by( &:id ).each do | ident |
      ancestor_of_community_taxon = obs.community_taxon&.ancestor_ids&.include?( ident.taxon_id )
      descendant_of_community_taxon = obs.community_taxon && ident.taxon.ancestor_ids.include?( obs.community_taxon_id )
      matches_community_taxon = obs.community_taxon && ( ident.taxon_id == obs.community_taxon_id )
      progressive = ( categories[:improving] + categories[:supporting] ).flatten.detect do | i |
        i.taxon.self_and_ancestor_ids.include?( ident.taxon_id )
      end.blank?
      if obs.community_taxon.blank? || descendant_of_community_taxon
    ...
    categories.each do | category, cat_idents |
      next if cat_idents.compact.blank?
  ...
  def self.run_update_curator_identification( ident )
    ...
    obs.project_observations.each do | po |
      next unless current_ident.user.project_users.exists?( ["project_id = ? AND role IN (?)", po.project_id,
                                                             [ProjectUser::MANAGER, ProjectUser::CURATOR]] )
      ...
      ProjectUser.delay( priority: INTEGRITY_PRIORITY,
        unique_hash: { "ProjectUser::update_observations_counter_cache_from_project_and_user":
          [po.project_id,
           obs.user_id] } ).update_observations_counter_cache_from_project_and_user( po.project_id, obs.user_id )
      ProjectUser.delay( priority: INTEGRITY_PRIORITY,
        unique_hash: { "ProjectUser::update_taxa_counter_cache_from_project_and_user":
          [po.project_id,
           obs.user_id] } ).update_taxa_counter_cache_from_project_and_user( po.project_id, obs.user_id )
      Project.delay(
        priority: INTEGRITY_PRIORITY,
  ...
  def self.run_revisit_curator_identification( observation_id, user_id )
    ...
    obs.project_observations.each do | po |
      other_curator_conditions = ["project_id = ? AND role IN (?)", po.project_id,
                                  [ProjectUser::MANAGER, ProjectUser::CURATOR]]
      # The ident that was deleted is owned by user who is a curator of a project that that obs belongs to
      next unless usr.project_users.exists?( other_curator_conditions )
  ...
  def self.update_for_taxon_change( taxon_change, options = {} )
    ...
    scope.find_each do | ident |
      ...
      if ident.disagreement && ident.previous_observation_taxon &&
          ( current_synonym = ident.previous_observation_taxon.current_synonymous_taxon )
    ...
    Identification.current.where( "disagreement AND previous_observation_taxon_id IN (?)",
      input_taxon_ids ).find_each do | ident |
  ...
  def self.update_disagreement_identifications_for_taxon( taxon )
    return if !taxon.is_a?( Taxon ) && !( taxon = Taxon.find_by_id( taxon ) )
  ...
  def self.reindex_for_taxon( taxon_id )
    ...
    loop do
      ...
      break unless r.response&.hits && r.response.hits.hits
  ...
  def self.merge_future_duplicates( reject, keeper )
    ...
    _, reflection = reflections.detect {| _k, r | r.klass == reject.class && r.macro == :belongs_to }
    sql = <<-SQL
      SELECT
        observation_id,
        array_agg(id) AS ids
