...
class ControlledTerm < ApplicationRecord
  ...
  scope :unassigned_values, lambda {
    where( is_value: true ).
  ...
  def term_label( options = {} )
    ...
    if options[:taxon].is_a?( Taxon )
      matching_label = all_labels.detect do | l |
        l.locale == options[:locale] &&
          l.valid_within_taxon &&
          options[:taxon].has_ancestor_taxon_id( l.valid_within_taxon.id )
      end
      return matching_label if matching_label
    end
    if ( matching_label = all_labels.detect {| l | l.locale == options[:locale] } )
  ...
  def label( options = {} )
    return unless ( tl = term_label( options ) )
  ...
  def values_for_observation( observation )
    values - observation.annotations.where( controlled_attribute: self ).map( &:controlled_value )
  end
