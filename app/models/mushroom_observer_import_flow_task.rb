...
class MushroomObserverImportFlowTask < FlowTask
  ...
  def get( url, params = {} )
    ...
    resp = begin
      log "GET #{url}, #{params}"
      RestClient.get( url, params: params, "User-Agent" => "iNaturalist/MushroomObserverImportFlowTask" )
  ...
  def get_results_xml( options = {} )
    ...
    raise ForbiddenError, "API key does not have permission to retrieve observations"
  end

  def mo_user_id( for_api_key = nil )
    unless @mo_user_id
      for_api_key ||= api_key
      xml = Nokogiri::XML( get( "https://mushroomobserver.org/api2/observations.xml", id: 0, api_key: for_api_key ) )
      @mo_user_id = xml.at( "response/user" ).try( :[], :id )
    end
    @mo_user_id
  rescue RestClient::Forbidden
    raise ForbiddenError, "API key is not valid"
  end

  def mo_user_name( _for_api_key = nil )
    ...
    raise ForbiddenError, "API key does not have permission to retrieve user info"
  end

  def api_key
    return nil unless inputs&.first && inputs.first.extra
  ...
  def taxon_for_name( name )
    ...
    taxon ||= begin
      Taxon.import( name, ancestor: Taxon::ICONIC_TAXA_BY_NAME["Fungi"] )
  ...
  def observation_from_result( result, options = {} )
    ...
    existing = Observation.joins( :observation_field_values ).
      where(
    ...
    if ( notes_fields = result.at_css( "> notes_fields" ) )
      o.description ||= ""
      notes_fields_hash = notes_fields.children.each_with_object( {} ) do | notes_field, memo |
    ...
    if !options[:skip_images] && ( images = images_from_result( result ) ) && images.size.positive?
      images.each do | image |
        ...
        if ( image_license = image.at( "license" ) )
          ...
          when %r{by-nc-sa/} then Photo::CC_BY_NC_SA
          when %r{by-nc-nd/} then Photo::CC_BY_NC_ND
          when %r{by-sa/} then Photo::CC_BY_SA
          when %r{by-nd/} then Photo::CC_BY_ND
          when %r{by/} then Photo::CC_BY
          when %r{publicdomain/} then Photo::PD
          end
        end
