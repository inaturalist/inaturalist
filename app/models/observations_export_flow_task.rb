...
class ObservationsExportFlowTask < FlowTask
  ...
  def must_have_query
    return unless params.keys.blank?

    errors.add( :base, :must_have_query )
  end

  def must_have_reasonable_number_of_rows
    return unless observations_count > MAX_OBSERVATIONS
  ...
  def run( run_options = {} )
    ...
    File.open( archive_path ) do | f |
      outputs.create!( file: f )
    end
    unless run_options[:keep_tmp_files]
  ...
  def csv_archive
    ...
    fpath = csv_path
    FileUtils.mkdir_p( File.dirname( fpath ), mode: 0o755 )
  ...
  def export_columns
    ...
    if exp_columns.is_a?( Hash ) || exp_columns.is_a?( ActionController::Parameters )
      exp_columns = exp_columns.to_h.select {| _k, v | v == "1" }.keys
    end
