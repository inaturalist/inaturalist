class UpdateAction < ActiveRecord::Base
  ...
  def self.email_updates_to_user(subscriber, start_time, end_time)
    logger.debug "email_updates_to_user: #{subscriber}"
    user = subscriber
    ...
    logger.debug "email_updates_to_user: #{subscriber}, updates: #{updates.size}"
    ...
    logger.debug "email_updates_to_user: #{subscriber}, emailing updates: #{updates.size}"
    Emailer.updates_notification(user, updates).deliver_now
    logger.debug "email_updates_to_user: #{subscriber} finished"
  end

  def self.logger
    @logger ||= Logger.new( "/var/log/update-email-updates.log", level: Logger::DEBUG )
  end
