...
class Observation < ActiveRecord::Base
  ...
  def munge_observed_on_with_chronic
    Observations::ParseObservedOnString.new(self).call
  end
  ...
  def self.place_guess_from_latlon( lat, lon, options = {} )
    Observations::GuessPlaceFromLatlon.new(lat, lon, options).call
  end
  ...
  def self.dedupe_for_user(user, options = {})
    Observations::DedupeForUser.new(user, options).call
  end
