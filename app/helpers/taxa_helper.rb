...
module TaxaHelper
  ...
  #
  def merge_taxa( taxa )
    if taxa.map( &:name ).uniq.size > 1
      raise "These taxa don't all have the same name!  Unique names: #{taxa.map( &:name ).uniq.join( ',' )}"
    end
    if taxa.map( &:rank ).uniq.size > 1
      raise "These taxa aren't all of the same rank!  Unique ranks: #{taxa.map( &:rank ).uniq.join( ',' )}"
    end

    if taxa.map( &:ancestors ).uniq.size > 1
    ...
    # merge the children
    orphans = sorted_taxa.map( &:children ).flatten
    orphans.each do | child |
      merged_taxon.children << child
    end

    # merge the taxon_names
    orphaned_names = sorted_taxa.map( &:taxon_names ).flatten
  ...
  def iconic_taxon_image_url( taxon, params = {} )
    ...
    path = "iconic_taxa/"
    path += if iconic_taxon
      iconic_taxon.name.downcase
    else
      "unknown"
    end
    path += "-#{params[:color]}" if params[:color]
    path += "-#{params[:size]}px" if params[:size]
    path += ".png"
    image_url( path )
  end
  ...
  def capitalize_piece( piece )
    ...
    if ( bits = piece.match( /^([a-z]['’])(\p{Word}+)/ ) ) || ( bits = piece.match( /^([ʻ])(\p{Word}+)/ ) )
  ...
  def capitalize_name( comname )
    ...
    comname_pieces.each_with_index.map do | piece, i |
      if i > 0 && uncapitalized.include?( piece.downcase )
        piece.downcase
      elsif i == comname_pieces.size - 1
        piece.split( "-" ).map do | s |
          uncapitalized.include?( s.downcase ) ? s.downcase : capitalize_piece( s )
        end.join( "-" )
      else
