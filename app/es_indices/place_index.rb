...
class Place < ApplicationRecord
  ...
  def as_indexed_json( _options = {} )
    ...
    I18N_SUPPORTED_LOCALES.each do | locale |
      ...
      next unless locale_name && locale_name != name
...

    {
      ...
      geometry_geojson: ( place_geometry&.persisted? && !index_without_geometry ) ?
        ElasticModel.geom_geojson( place_geometry.simplified_geom ) : nil,
      bounding_box_geojson: ( place_geometry&.persisted? && !index_without_geometry ) ?
      ...
      without_check_list: check_list_id.blank? || nil,
  ...
  def double_check_index
    return if destroyed?
    return if geom_in_elastic_index?

    original_index_without_geometry = index_without_geometry
