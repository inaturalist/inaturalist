...
class AssessmentsController < ApplicationController
  ...
  def index
    ...
    @assessments = Assessment.where( project_id: @project.id ).includes( :taxon,
      :sections ).order( "taxa.name ASC" ).
    ...
    respond_to do | format |
      format.html do
        ...
        @comment_counts = Comment.group( :parent_id ).where( "parent_type = 'AssessmentSection' AND parent_id IN (?)",
          section_ids ).count
        ...
        statuses = ConservationStatus.
          select( "max(conservation_statuses.id), authority, status" ).
          joins( taxon: :assessments ).
          where( "assessments.project_id = ?", @project ).
          group( :authority, :status )
        @conservation_statuses = statuses.each_with_object( {} ) do | status, memo |
  ...
  def project_curator_required
    return if @project.curated_by?( current_user )
