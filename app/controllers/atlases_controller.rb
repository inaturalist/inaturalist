...
class AtlasesController < ApplicationController
  ...
  before_action :curator_required, only: [
    :alter_atlas_presence,
    :create,
    :destroy,
    :destroy_all_alterations,
    :edit,
    :new,
    :refresh_atlas,
    :remove_atlas_alteration,
    :remove_listed_taxon_alteration,
    :update
  ]
  ...
  def show
    ...
    @atlas_presence_places_with_establishment_means_hash = @atlas_presence_places_with_establishment_means.map do | e |
      [e[:id], e[:establishment_means]]
    end.to_h
    @exploded_places = @atlas.exploded_atlas_places.map {| e | [e.place_id, e.id] }.to_h
    @atlas_presence_places_with_establishment_means_hash_json =
      @atlas_presence_places_with_establishment_means_hash.to_json
    ...
    @num_obs_not_in_atlas_places = INatAPIService.
      observations( @observations_not_in_atlas_places_params.merge( per_page: 0 ) ).
      total_results
    respond_to do | format |
      format.html do
        @is_curator = current_user.is_curator?.to_s
  ...
  def alter_atlas_presence
    ...
    unless presence
      comprehensive_list = place.check_lists.where( "comprehensive AND lists.taxon_id IN (?)",
        taxon.self_and_ancestor_ids ).first
    ...
    respond_to do | format |
      format.json do
        if error
          render json: {
            place_name: place.try_methods( :display_name, :name ),
            place_id: place_id,
            presence: presence,
            error: error
