...
class TaxonChangesController < ApplicationController
  ...
  before_action :ensure_can_vote!, only: :like
  ...
  def index
    ...
    id = filter_params[:ancestor_taxon_id]
    @ancestor_taxon = Taxon.find_by_id( id.to_i ) unless id.blank?
    @change_group = filter_params[:change_group] unless filter_params[:change_group].blank?
    id = filter_params[:taxon_scheme_id]
    @taxon_scheme = TaxonScheme.find_by( id: id ) if id.present?
    ...
    @change_groups = TaxonChange.select( :change_group ).group( :change_group ).
      map( &:change_group ).compact.sort
    @taxon_schemes = TaxonScheme.limit( 100 ).sort_by( &:title )
  ...
  def show
    ...
    if @taxon_change.committed?
      ...
      changes = @taxon_change.input_taxa.flat_map {| it | TaxonChange.input_taxon( it ).to_a }
      changes.uniq!
      @existing = changes.reject {| tc | tc.id == @taxon_change.id || tc.committed? }
      @upstream_taxon_framework = @taxon_change.input_taxa.detect( &:upstream_taxon_framework ).
        try( :upstream_taxon_framework )
      if current_user && @upstream_taxon_framework && @upstream_taxon_framework.taxon_curators.count.positive? &&
          @upstream_taxon_framework.taxon_curators.reject {| tc | tc.user_id == current_user }
        @curated_upstream_taxon_framework = @upstream_taxon_framework
      end
      @observose_taxon = @taxon_change.input_taxa.detect( &:observose_branch? ).try( :observose_branch? )
      @observose_taxon_threshold = Taxon::NUM_OBSERVATIONS_REQUIRING_ADMIN_TO_COMMIT_TAXON_CHANGES
      @observose_taxon_warning = @taxon_change.input_taxa.detect( &:observose_warning_branch? ).
  ...
  def new
    @change_groups = TaxonChange.select( :change_group ).group( :change_group ).map( &:change_group ).compact.sort
    @klass = params[:type].to_s.safe_constantize
    @klass = TaxonSwap unless @klass.is_a?( Class ) && @klass <= TaxonChange
  ...
  def create
    change_params = get_change_params
    unless change_params
      flash[:error] = t( "views.taxon_changes.no_changes_specified" )
    ...
    if @taxon_change.save
      flash[:notice] = t( "views.taxon_changes.taxon_change_successfully_created" )
  ...
  def update
    ...
    if @taxon_change.update( change_params )
      flash[:notice] = t( "views.taxon_changes.taxon_change_successfully_update" )
      redirect_to taxon_change_path( @taxon_change )
    else
      @change_groups = TaxonChange.select( :change_group ).
        group( :change_group ).map( &:change_group ).compact.sort
  ...
  def destroy
    ...
    if @taxon_change.destroy
      flash[:notice] = t( "views.taxon_changes.taxon_change_deleted" )
    else
      flash[:error] = t( "views.taxon_changes.something_whent_wrong", id: @taxon_change.id )
    end
  ...
  def commit
    if @taxon_change.committed?
      flash[:error] = t( "views.taxon_changes.taxon_change_already_committed" )
    ...
    if @taxon
      flash[:error] = t(
        "views.taxon_changes.output_taxon_rank_level_not_coarser_html",
        taxon_link: view_context.link_to( @taxon.name, @taxon )
      )
      redirect_back_or_default( taxon_changes_path )
      return
    end
    ...
    if !@taxon_change.move_children? && @taxon_change.active_children_conflict?
      flash[:error] = t( "views.taxon_changes.input_taxon_cannot_have_active_children" )
      redirect_back_or_default @taxon_change
      return
    end
  ...
  def commit_for_user
    ...
    @records = current_user.send( @reflection.name ).
      where( "#{@reflection.table_name}.taxon_id IN (?)", taxon_ids ).page( params[:page] )
  ...
  def commit_records
    ...
    if params[:record_id]
      @record = current_user.send( @reflection.name ).
        where( "#{@reflection.table_name}.id = ?", params[:record_id] ).first
      unless @record
      ...
      @records = current_user.send( @reflection.name ).
        where( "#{@reflection.table_name}.id IN (?)", params[:record_ids] ).to_a
      if @records.blank?
  ...
  def group
    ...
    [
      ...
      input_counts[id] = input_counts.fetch( id, 0 ) + 1
    end
    duplicate_input_ids = input_counts.filter_map do | id, count |
      Rails.logger.debug { "[id, count]: #{[id, count].inspect}" }
      id if count > 1
    end
  ...
  def analyze_ids
    ...
    if @taxon_change && @taxon_change.type == "TaxonSplit" &&
        @taxon_change.taxon && @taxon_change.taxon_change_taxa.size > 1
  ...
  def like
    if current_user.voted_up_on?( @taxon_change )
      @taxon_change.unvote_by current_user
      flash[:notice] = t( "views.taxon_changes.vote_removed" )
    else
      @taxon_change.upvote_from current_user
      flash[:notice] = t( "views.taxon_changes.vote_added" )
    end
  ...
  def ensure_can_vote!
    unless current_user&.is_curator? && current_user.id != @taxon_change.user_id
      head :forbidden and return
    end
    return if @taxon_change.status_pending? || @taxon_change.status_approved?
  ...
  def load_taxon_change
    @taxon_change = TaxonChange.where( id: params[:id] || params[:taxon_change_id] ).
      includes(
    ...
    render_404 unless @taxon_change
  end

  def stub_taxon_split
    input_taxon = Taxon.where( id: params[:input_taxon_id].to_i ).first
    output_taxa = Taxon.where( id: params[:output_taxon_ids].map( &:to_i ) )
  ...
  def load_user_content_info
    ...
    has_many_reflections = User.reflections.select {| _, v | v.macro == :has_many }
  ...
  def identification_analysis_with_url( row, label )
    ...
    if label == :total_id_count
      { name: row[:name], taxon_id: row[:taxon_id], taxon_url: taxon_url, id_count: row[:id_count], url: url }
    else
      role = nil
      if row[:atlas_id].nil?
        ...
        atlas_string = row[:atlas_active] ? t( :atlased ) : t( :not_atlased )
