...
class TaxonNamesController < ApplicationController
  ...
  def taxon
    @taxon_names = @taxon.taxon_names.sort_by {| tn | [tn.position, tn.id] }.reject( &:is_scientific? )
    @place_taxon_names = PlaceTaxonName.joins( :taxon_name, :place ).where( "taxon_names.taxon_id = ?",
      @taxon ).sort_by( &:position )
    place_taxon_name_tn_ids = @place_taxon_names.map( &:taxon_name_id )
    @names_by_place = @place_taxon_names.group_by( &:place ).sort_by {| k, _v | k.name }
    @global_names = @taxon_names.reject {| tn | place_taxon_name_tn_ids.include?( tn.id ) }
    respond_to do | format |
      format.html
    end
  end
  ...
  def load_lexicons
    @lexicons = Rails.cache.fetch( "lexicons", expires_in: 1.day ) do
      ...
      ].flatten.uniq.reject( &:blank? ).sort
    end
  end
end
