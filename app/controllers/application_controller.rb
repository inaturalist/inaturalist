...
class ApplicationController < ActionController::Base
  ...
  PER_PAGES = [10, 30, 50, 100, 200].freeze
  HEADER_VERSION = 21
  ...
  def set_ga_trackers
    ...
    return true if current_user&.prefers_no_tracking?
  ...
  def sign_out_spammers
    return unless current_user&.spammer?
  ...
  def curator_required
    return if logged_in? && current_user.is_curator?
  ...
  def current_user
    ...
    return nil unless doorkeeper_token&.accessible?
  ...
  def check_user_last_active
    ...
    return unless current_user.subscriptions_suspended_at
  ...
  def load_record( options = {} )
    ...
    record ||= begin
      klass.find( params[:id] || params["#{class_name}_id"] )
    rescue StandardError
      nil
    end
  ...
  def require_owner( options = {} )
    ...
    return if logged_in? && ( current_user.id == record.user_id || current_user.is_admin? )
  ...
  def require_guide_user
    return if logged_in? && @guide.editable_by?( current_user )
  ...
  def limit_page_param_for_search
    return unless !params[:page].blank? && params[:page].to_i > 50
  ...
  def current_preferences( update_params = nil )
    ...
    prefs = if logged_in?
      ...
      session[:preferences] ||= User.preference_definitions.each_with_object( {} ) do | pair, memo |
        name, pref = pair
        memo[name] = pref.default_value
      end
    end

    update_params = update_params.to_h.reject {| _k, v | v.blank? } if update_params
  ...
  def admin_or_this_site_admin_required
    return if logged_in? && ( current_user.is_admin? || ( @site && current_user.is_site_admin_of?( @site ) ) )

    only_admins_failure_state
  end

  def admin_or_any_site_admin_required
    return if logged_in? && ( current_user.is_admin? || current_user.site_admins.any? )
  ...
  def login_from_param
    ...
    provider, url = ProviderAuthorization::AUTH_URLS.detect do | provider, _url |
  ...
  def authenticate_with_oauth?
    ...
    return false if jwt_claims&.fetch( "user_id" )
  ...
  def set_session_oauth_application_id
    if doorkeeper_token&.accessible? && ( a = doorkeeper_token.try( :application ) )
