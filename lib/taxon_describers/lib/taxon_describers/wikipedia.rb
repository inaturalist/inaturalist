  ...
  class Wikipedia < Base
    ...
    def describe( taxon )
      ...
      if Rails.env.production?
        ...
        wikipedia_url = wikidata_wikipedia_url_for_taxon( taxon )
        if wikipedia_url
      ...
      content + "<p class='inat-wikipedia-attribution'>#{
        I18n.t( :wikipedia_attribution_cc_by_sa_3_html, url: page_url( taxon ), title: page_title )
      }</p>"
    end
    ...
    def page_url( taxon )
      ...
      url = default_page_url( taxon ) if url.blank?
      url
    end
