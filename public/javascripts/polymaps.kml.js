  ...
  po.kml = function() {
    ...
    var types = {
      ...
      MultiGeometry: function(e) {
        var coordinates = []
        var outerBoundaries = e.getElementsByTagName('outerBoundaryIs')
        for (var i = outerBoundaries.length - 1; i >= 0; i--){
          var poly = outerBoundaries[i].getElementsByTagName("coordinates")[0]
            .textContent
            .trim()
            .split(/\s+/)
            .map(function(a) { return a.split(",").slice(0, 2).map(Number); });
