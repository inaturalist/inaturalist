(function($){
  ...
  function findFormField(context, name, options) {
    ...
    var field = $(context).parents('form').find(tagName+'[name="'+name+'"]:first');
    if ($(field).length == 0) {
  ...
  function setExact(isExact) {
    logMessage("DEBUG: Fired set exact.  isExact: ", isExact);
  ...
  $.fn.latLonSelector.updateFormLatLon = function(lat, lon) {
    var input = $.fn.latLonSelector._currentInput;
    logMessage("IM IN YR updateFormLatLon");
