...
describe FlickrPhoto, "creation" do
  ...
  before(:each) do
    ...
    @cc_flickr_photo = FlickrPhoto.new_from_api_response(fp, :user => users(:quentin))
    
    # pic of a flower Ken-ichi has marked all rights reserved
    fp = @flickr.photos.getInfo(:photo_id => 2394365945)
    @noncc_flickr_photo = FlickrPhoto.new_from_api_response(fp, :user => users(:quentin))
  end
  ...
  it "should not save if there is no assoc'd iNat user and the pic isn't CC" do
    ...
    @noncc_flickr_photo.errors[:license].should be_blank
  end
  ...
  it "should not be valid if the associated user didn't take the photo" do
    fp = @flickr.photos.getInfo(:photo_id => 469509092)
    photo_by_nate = FlickrPhoto.new_from_api_response(fp, :user => users(:quentin))
...
describe FlickrPhoto, "to_observation" do
  ...
  before(:all) do
    ...
    @fp = @flickr.photos.getInfo(:photo_id => 3372373404)
  end
  
  before(:each) do
    @flickr_photo = FlickrPhoto.new_from_api_response(@fp, :user => users(:quentin))
  end
