    ...
    describe "elastic_search" do
      it "searches for match_all: { } as a wildcard query" do
        expect(Observation.__elasticsearch__).to receive(:search).with(
          { query: { match_all: { } } }, { preference: "_local" }).and_return(true)
      ...
      it "adds matches to the query" do
        expect(Observation.__elasticsearch__).to receive(:search).with(
          { query: { bool: {
            must: [ { match: { id: 5 } } ] } } }, { preference: "_local" }).and_return(true)
      ...
      it "adds terms matches to the query" do
        expect(Observation.__elasticsearch__).to receive(:search).with(
          { query: { bool: {
            must: [ { terms: { id: [ 1, 3 ] } } ] } } }, { preference: "_local" }).and_return(true)
      ...
      it "adds envelope filters" do
        expect(Observation.__elasticsearch__).to receive(:search).with(
          ...
          { preference: "_local" }).and_return(true)
      ...
      it "adds sorts to the query" do
        expect(Observation.__elasticsearch__).to receive(:search).with(
          { query: { match_all: { } },
            sort: { score: :desc } }, { preference: "_local" }).and_return(true)
      ...
      it "allows certain fields to be specified" do
        expect(Observation.__elasticsearch__).to receive(:search).with(
          { query: { match_all: { } },
            fields: [ :id, :description ] }, { preference: "_local" }).and_return(true)
      ...
      it "adds aggregations to the query" do
        expect(Observation.__elasticsearch__).to receive(:search).with(
          { query: { match_all: { } },
            aggs: { colors: { terms: {
              field: :"colors.id", size: 10 } } } }, { preference: "_local" }).and_return(true)
